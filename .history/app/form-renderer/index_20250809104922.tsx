...
// ✅ REFACTORED: Central mapping for all pixel events
const pixelEventMap = {
  facebook: {
  ...
  pinterest: {
    ...
    events: {
      ...
      AddPaymentInfo: 'checkout', // Pinterest uses 'checkout' for this stage too
      Purchase: 'checkout',
      AddToCart: 'addtocart',
    },
...
const trackEvent = (eventName: 'InitiateCheckout' | 'AddPaymentInfo' | 'Purchase' | 'AddToCart', eventData: any = {}) => {
  ...
  for (const [pixel, id] of Object.entries(pixelConfigs)) {
    if (id) {
      ...
      // Check if a specific condition must be met to send the event
...
const FormApp = () => {
  ...
  // ✅ FIXED: Fetch cart data using polling to prevent infinite loops
  useEffect(() => {
    let isMounted = true;
  ...
  // The renderField function remains the same as it was already well-structured.
  // ... (Paste your existing renderField function here to keep the file complete)
