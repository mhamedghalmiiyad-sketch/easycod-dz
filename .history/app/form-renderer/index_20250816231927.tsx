...
// --- INTERFACES (No changes needed here, they are well-defined) ---
...
// Note: The global config retrieval is now handled by the initializer.
...
const FormApp = ({ config }: { config: Config }) => {
  // --- START PIXEL/EVENT TRACKING LOGIC (Depends on 'config' prop) ---
  ...
  // --- END PIXEL/EVENT TRACKING LOGIC ---
...
function initializeForm(container: HTMLElement) {
  ...
  try {
    // 1. Get the base config from the metafield
    const mainConfig: Config = JSON.parse(configElement.textContent || '{}');

    // 2. Get section-specific settings from the data attribute
    const sectionSettings = sectionSettingsJSON ? JSON.parse(sectionSettingsJSON) : {};

    // 3. Merge Theme Editor settings into the main config
    if (sectionSettings.heading_override?.trim()) {
      const headerField = mainConfig.formFields.find(f => f.type === 'header' || f.type === 'section-header');
      if (headerField) {
        (headerField.settings as CustomTextSettings).text = sectionSettings.heading_override;
      }
    }
    if (sectionSettings.button_text_override?.trim()) {
      const submitField = mainConfig.formFields.find(f => f.type === 'submit');
      if (submitField) {
        (submitField.settings as ButtonSettings).text = sectionSettings.button_text_override;
      }
    }

    // 4. Render the React app with the final merged config
