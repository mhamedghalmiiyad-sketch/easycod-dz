  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    switch (field.type) {
      ...
      case 'custom-icon-feature': {
        ...
        return (
          <Box key={field.id} sx={{ my: 2, py: 2, width: '100%' }}>
            {s.title && s.titlePosition === 'top' && (
              <Typography variant="h6" component="h3" textAlign="center" sx={{ mb: 2.5, fontWeight: 600 }}>
                {s.title}
              </Typography>
            )}
            ...
            <Box
              sx={{
                display: 'grid',
                gridTemplateColumns: `repeat(${enabledFeatures.length}, 1fr)`,
                gap: 2,
                alignItems: 'start',
              }}
            >
            ...
            {s.title && s.titlePosition === 'bottom' && (
                <Typography variant="h6" component="h3" textAlign="center" sx={{ mt: 2.5, fontWeight: 600 }}>
                    {s.title}
                </Typography>
            )}
          </Box>
