...
<body>
  <div class="easycod-form-container" data-section-id="form1"></div>
  <script id="easycod-form-config-form1" type="application/json">
    {
      "formFields": [
        {
          "id": "firstName",
          ...
          "editable": true,
          "label": "First Name",
          "settings": { "required": true, "placeholder": "Enter your first name" }
        },
        {
          "id": "submit",
          "type": "submit",
...
const FormApp = ({ config }: { config: Config }) => {
  ...
  useEffect(() => {
    ...
    const fetchCart = async () => {
      try {
        // Use the shop domain from the current URL
        const currentUrl = new URL(window.location.href);
        const shopDomain = currentUrl.searchParams.get('shop') || currentUrl.hostname;

        const response = await fetch(`https://${shopDomain}/cart.js`);
        ...
        // Fallback: try to get cart data from URL parameters if available
        const urlParams = new URLSearchParams(window.location.search);
        const cartParam = urlParams.get('cart_data');
        if (cartParam && isMounted) {
          try {
            const fallbackCart = JSON.parse(decodeURIComponent(cartParam));
            setCart(fallbackCart);
          } catch (e) {
            console.error('Failed to parse fallback cart data:', e);
          }
        }
      } finally {
  ...
  useEffect(() => {
    ...
    if (hasWilayaField) {
      ...
      fetch('/apps/codform/proxy/algeria-locations')
        .then(res => {
          if (!res.ok) {
  ...
  useEffect(() => {
    ...
    if (hasCommuneField && selectedWilayaId) {
      setIsLoadingLocations(true);
      fetch(`/apps/codform/proxy/algeria-locations?wilaya_id=${selectedWilayaId}`)
        .then(res => {
          if (!res.ok) {
            throw new Error(`HTTP error! status: ${res.status}`);
          }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const currentUrl = new URL(window.location.href);
      const submitUrl = currentUrl.origin + currentUrl.pathname + currentUrl.search;
