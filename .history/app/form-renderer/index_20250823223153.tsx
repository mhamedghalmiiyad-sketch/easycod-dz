...
const FormApp = ({ config }: { config: Config }) => {
  ...
  useEffect(() => {
    ...
    // **UPDATED** Simplified Cart Fetching
    useEffect(() => {
        let isMounted = true;
  ...
  // **UPDATED** Fetch Wilayas when component mounts
  useEffect(() => {
    const hasWilayaField = config.formFields.some(f => f.type === 'wilaya' && f.enabled);
    if (hasWilayaField) {
      setIsLoadingLocations(true);
      fetch('/apps/proxy/algeria-locations') // Updated URL
        .then(res => {
          if (!res.ok) {
  ...
  // **UPDATED** Fetch Communes when Wilaya changes
  useEffect(() => {
    ...
    if (hasCommuneField && selectedWilayaId) {
      setIsLoadingLocations(true);
      fetch(`/apps/proxy/algeria-locations?wilaya_id=${selectedWilayaId}`)  // Updated URL
        .then(res => {
          if (!res.ok) {
            throw new Error(`HTTP error! status: ${res.status}`);
          }
  ...
  }, [formValues.wilaya, config.formFields]); // Depend on wilaya ID specifically
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // **UPDATED** Add session ID to form data
      ...
      if (response.ok) {
        ...
        try {
          ...
          if (cart && result.orderId && result.purchaseData) {
              trackEvent('Purchase', {
          ...
          const shopDomain = new URL(config.submitUrl).hostname;
          window.location.href = `https://${shopDomain}/pages/thank-you`;
          return;
        }
      } else {
        try {
  ...
  const FormMarkup = (
    <Box
      ...
      action={config.submitUrl} // <-- AND THIS LINE
...
/**
 * Finds all form containers on the page and initializes a React app for each.
 * This approach supports multiple instances of the form on the same page.
 */
document.addEventListener('DOMContentLoaded', () => {
  // 1. Find all potential form containers by their shared class name.
  const allFormContainers = document.querySelectorAll('.easycod-form-container');

  if (allFormContainers.length === 0) {
    console.warn("EasyCod Form: No form containers with class '.easycod-form-container' were found on this page.");
    return;
  }
  ...
  allFormContainers.forEach((container) => {
    try {
      // 3. Get the unique section ID from the container's data attribute.
      const sectionId = (container as HTMLElement).dataset.sectionId;
      if (!sectionId) {
        throw new Error("Container is missing the 'data-section-id' attribute.");
      }

      // 4. Construct the dynamic ID for the corresponding config script.
      const configElementId = `easycod-form-config-${sectionId}`;
      const configElement = document.getElementById(configElementId);

      if (!configElement || !configElement.textContent) {
        throw new Error(`Configuration data script with ID '${configElementId}' not found or is empty.`);
      }

      // 5. Parse the configuration and render the React app into this specific container.
      ...
      console.log(`✅ EasyCod Form successfully initialized for section ${sectionId}.`);

    } catch (error) {
      console.error("❌ Failed to initialize an EasyCod form instance:", error);
      container.innerHTML = '<p style="color: red; font-family: sans-serif; text-align: center;">Error: Could not load the form. Please check the console for details.</p>';
    }
  });
});