...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (wilayaIdParam) {
    try {
      // ✅ FIX: Prisma client uses camelCase (algeriaCities) for model names.
  ...
  try {
    ...
    // ✅ FIX: Added an explicit type for the 'w' parameter.
    const wilayasWithPrices = wilayasData.map((w: { wilaya_code: string; wilaya_name: string; wilaya_name_ascii: string; }) => {
