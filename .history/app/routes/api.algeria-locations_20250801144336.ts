 ...
 * }
 *
 * Then, run `npx prisma db pull` and `npx prisma generate`. The Prisma client will
 * likely generate the model name as `algeriaCities`.
 */
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    // Map the grouped data to a clean format that the front-end can use.
