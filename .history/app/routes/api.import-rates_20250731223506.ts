...
// Define the expected structure for the Noest API response.
interface NoestDeliveryFee {
  wilaya_id: number;
  name: string;
  tarif: string; // e.g., "1000.00"
  tarif_stopdesk: string; // e.g., "500.00"
}

// Define the expected structure for the EcoTrack API response.
interface EcoTrackWilaya {
  id: number;
  name: string;
  price: string; // This is the home delivery fee
  stop_desk_price: string;
}
  ...
  try {
    ...
    switch (apiProvider) {
      case "noest": {
        ...
        const feesObject: Record<string, NoestDeliveryFee> = data.tarifs?.delivery ?? {};

        // Map the Noest response to our standard DeliveryRate format.
        deliveryRates = Object.values(feesObject).map((fee) => {
          const wilayaInfo = wilayaInfoMap.get(fee.wilaya_id);
          return {
            wilaya_id: fee.wilaya_id,
            wilaya_name_fr: wilayaInfo?.name_fr ?? fee.name, // Use map, fallback to API name
            wilaya_name_ar: wilayaInfo?.name_ar ?? `ولاية ${fee.wilaya_id}`, // Fallback for Arabic
            home_fee: parseFloat(fee.tarif),
            desk_fee: parseFloat(fee.tarif_stopdesk),
          };
        });
        break;
      }
      ...
      case "ecotrack": {
        ...
        const ECOTRACK_API_URL = "https://api.ecotrack.dz/v1/wilayas";
        ...
        const data = await response.json();
        const wilayas: EcoTrackWilaya[] = data.data ?? [];

        // Map the EcoTrack response to our standard DeliveryRate format.
        deliveryRates = wilayas.map((wilaya) => {
          const wilayaInfo = wilayaInfoMap.get(wilaya.id);
          return {
            wilaya_id: wilaya.id,
            wilaya_name_fr: wilayaInfo?.name_fr ?? wilaya.name, // Use map, fallback to API name
            wilaya_name_ar: wilayaInfo?.name_ar ?? `ولاية ${wilaya.id}`, // Fallback for Arabic
            home_fee: parseFloat(wilaya.price),
            desk_fee: parseFloat(wilaya.stop_desk_price),
          };
        });
        break;
      }
    ...
    // 4. Return the standardized rates on success.
    console.log(`Successfully imported ${deliveryRates.length} rates for ${apiProvider}.`);
    return json({ success: true, rates: deliveryRates });
