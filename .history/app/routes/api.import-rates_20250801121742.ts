...
case "ecotrack": {
  ...
  const apiUrl = url.searchParams.get("apiUrl") || "https://app.ecotrack.dz"; // Default URL
  ...
  const data = await response.json();
  // EcoTrack returns data in the "livraison" array for delivery fees
  ...
  deliveryRates = livraisonRates.map((rate: any) => {
    ...
    return {
      ...
      // Use 'tarif' and 'tarif_stopdesk' to match the expected format in your frontend
      tarif: rate.tarif,
      tarif_stopdesk: rate.tarif_stopdesk,
    };
  });
  break;
}
