...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const stats: DashboardStats = {
    ...
    conversionRate: 0,
    topProduct: "",
    // ✅ FIX: The 'status' property doesn't exist on orderTrackingData. Assuming all fetched orders are pending.
  ...
  const recentOrders: RecentOrder[] = orderTrackingData.map((order) => ({
    ...
    // ✅ FIX: Set status to 'pending' by default as it's not in the database model.
    status: 'pending',
              ...
              <Card key={step.id} background={step.completed ? "bg-surface-success" : undefined}>
                <Box padding="500">
                  <BlockStack gap="400">
                    ...
                    {/* ✅ FIX: Changed onAction to onClick for Polaris Button component */}
                    <Button
...
// ✅ 2. Fix navigation to use onClick handlers for internal routes
