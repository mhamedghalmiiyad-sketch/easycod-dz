...
const generalSettingsSchema = z.object({
  ...
  abandonedCartDelayMinutes: z.string().regex(/^\d+$/, "Must be a number").refine(val => {
    const num = parseInt(val);
    return num >= 5 && num <= 1440;
  }, "Must be between 5 and 1440 minutes"),
  abandonedCartMaxReminders: z.string().regex(/^\d+$/, "Must be a number").refine(val => {
    const num = parseInt(val);
    return num >= 1 && num <= 10;
  }, "Must be between 1 and 10 reminders"),
  abandonedCartReminderIntervalHours: z.string().regex(/^\d+$/, "Must be a number").refine(val => {
    const num = parseInt(val);
    return num >= 1 && num <= 168;
  }, "Must be between 1 and 168 hours"),
  abandonedCartEmailSubject: z.string().max(255, "Subject is too long (max 255 characters)."),
  abandonedCartEmailTemplate: z.string().max(5000, "Email template is too long (max 5000 characters)."),
  abandonedCartWhatsAppTemplate: z.string().max(500, "WhatsApp template is too long (max 500 characters)."),
  ...
  try {
    ...
    if (actionType === "reset") {
      ...
      const rawSettings = {
        ...
        abandonedCartRecoveryMethod: formData.get("abandonedCartRecoveryMethod") as "email" | "whatsapp" | "both",
      };
