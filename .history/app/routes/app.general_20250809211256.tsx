...
const getCachedSettings = async (shopDomain: string): Promise<GeneralSettings> => {
  const cacheKey = `settings:${shopDomain}`;
  const cached = settingsCache.get(cacheKey);

  if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
    console.log("📦 Using cached settings for", shopDomain);
    return cached.data;
  }
  ...
  try {
    ...
    const settings = await db.shopSettings.findUnique({
      where: { shopId: shopDomain } // ✅ CORRECT: Use shopDomain for the query
    });
    ...
    if (settings?.generalSettings) {
      ...
      console.log("No settings found, using defaults for", shopDomain);
      parsedSettings = DEFAULT_SETTINGS;
    }
    ...
    console.error("Error fetching settings from database for", shopDomain, error);
...
const resetAllAppData = async (sessionId: string, shopDomain: string) => {
  ...
  try {
    // ✅ Server-only imports are moved inside the function that uses them
    const { default: db } = await import("../db.server");
    const { initializeShopSettings } = await import("../utils/shopSettings.server");
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  // ✅ CORRECT: Pass the permanent shop domain, not the temporary session ID
  await initializeShopSettings(session.shop); 
  const generalSettings = await getCachedSettings(session.shop); 
...
export const action = async ({ request }: ActionFunctionArgs) => {
  try {
    ...
    if (actionType === "reset") {
      // ✅ CORRECT: Use shop domain for reset logic
      await resetAllAppData(session.shop);
      settingsToSave = DEFAULT_SETTINGS;
      console.log("🔄 Complete database reset performed");
    } else {
      // ✅ CORRECT: Use shop domain for initialization
      await initializeShopSettings(session.shop);
    ...
    // ✅ CORRECT: Use shop domain for database operations
    ...
    await db.shopSettings.upsert({
      ...
      create: {
        // Correctly associate with the Session model using the session.id
        Session: { connect: { id: session.id } },
        shopId: session.shop, // Explicitly set shopId which is the primary key
        generalSettings: JSON.stringify(settingsToSave),
        // Add other required default fields for creation
        formFields: "[]",
        formStyle: "{}",
        pixelSettings: "{}",
        userBlocking: "{}",
    ...
    // ✅ CORRECT: Use shop domain for cache invalidation
    settingsCache.delete(`settings:${session.shop}`);

    // ✅ CORRECT: Use shop domain for logging (and remove session.id)
    ...
    const message = actionType === "reset"
      ? "All app data has been reset to factory defaults."
