...
const resetAllAppData = async (sessionId: string, shopDomain: string) => {
  ...
  try {
    ...
    // Call initialize with the permanent shop domain for consistency
...
export const action = async ({ request }: ActionFunctionArgs) => {
  try {
    ...
    if (actionType === "reset") {
      // ✅ FIX: Pass both session.id and session.shop as required by the function
    ...
    await db.shopSettings.upsert({
      ...
      create: {
        // ✅ FIX: Removed the conflicting 'Session' relation.
        // The settings are tied to the shop via 'shopId', which is correct.
        shopId: session.shop, // This is the primary key.
        generalSettings: JSON.stringify(settingsToSave),
    ...
    // ✅ FIX: Add the missing 'sessionId' argument to the function call.
