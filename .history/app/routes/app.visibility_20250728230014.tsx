...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    const settings = await db.shopSettings.findUnique({
      where: { shopId: session.shop }, // Changed from session.id
    });
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    const result = await db.shopSettings.upsert({
      where: {
        shopId: session.shop // Changed from session.id
      },
      update: {
        ...dataToSave,
        updatedAt: new Date(),
                    ...
                    <BlockStack gap="500">
                      ...
                      <BlockStack gap="300">
                        <Checkbox
                          ...
                          onChange={(checked) => setEnableSpecificProductsState(checked)}
                        />
                      ...
                      <BlockStack gap="300">
                        <Checkbox
                          ...
                          onChange={(checked) => setDisableSpecificProductsState(checked)}
                        />
