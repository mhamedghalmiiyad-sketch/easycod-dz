...
// Extend HTML elements to include the custom ui-save-bar element
// This allows TypeScript to recognize the element and its methods.
declare global {
  namespace JSX {
...
// It provides default values if no settings are found to ensure the app is stable.
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    // If no settings exist in the database, return a default configuration.
    ...
    // If settings are found, parse the JSON fields and merge with defaults
    // to prevent errors from missing fields.
    return json({
    ...
    // In case of an error, return default values to ensure the page still loads.
    return json({
        visibilityMode: "both_cart_product",
        visibleProducts: [],
        hiddenProducts: [],
...
// It uses `upsert` to either create new settings or update existing ones.
export const action = async ({ request }: ActionFunctionArgs) => {
  const { session } = await authenticate.admin(request);
  const formData = await request.formData();

  try {
    // Extract and format all data from the form submission.
    const dataToSave = {
    ...
    // Use Prisma's upsert to create or update the settings for the current shop.
...
// This ensures consistent JSON.stringify results for change detection.
...
export default function VisibilityPage() {
  ...
  const [formState, setFormState] = useState<VisibilityFormState>(loaderData);
  // Store the initial state to compare against for unsaved changes.
  const [initialFormState, setInitialFormState] = useState<VisibilityFormState>(loaderData);

  const [showToast, setShowToast] = useState(false);
  const saveBarRef = useRef<HTMLElement>(null);

  // Compare initial and current form states to detect unsaved changes.
  ...
  const handleSave = useCallback(() => {
    ...
    // Append all form state fields to the FormData object.
  ...
  useEffect(() => {
    const saveBarElement = saveBarRef.current as any; // Cast to any to access show/hide
  ...
  useEffect(() => {
    if (actionData?.success && !isLoading) {
      setShowToast(true);
      // After a successful save, update the initial state to the current state.
      // This resets `hasUnsavedChanges` to false and hides the save bar.
  ...
  // Handlers for opening resource pickers (Products, Countries).
  // In a real app, this would use Shopify App Bridge.
  ...
  // Renders a preview of selected items (products or countries).
  ...
  return (
    <Frame>
      {/* The Save Bar component, controlled by the `hasUnsavedChanges` state. */}
      <ui-save-bar ref={saveBarRef} discardConfirmation>
        <button className="Polaris-Button Polaris-Button--primary" onClick={handleSave} disabled={isLoading}>
          {isLoading ? "Saving..." : "Save"}
        </button>
        <button className="Polaris-Button" onClick={handleDiscard} disabled={isLoading}>
          Discard
        </button>
      </ui-save-bar>
