...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    // Use the utility function to save settings - note: you may need to extend upsertShopSettings 
    // to handle these additional fields or store them as JSON strings
    await upsertShopSettings(session.shop, {
      visibilityMode,
      visibleProducts: JSON.stringify(JSON.parse(visibleProducts || "[]")), // Store as JSON string
      hiddenProducts: JSON.stringify(JSON.parse(hiddenProducts || "[]")), // Store as JSON string
      allowedCountries: JSON.stringify(JSON.parse(allowedCountries || "[]")), // Store as JSON string
      minimumAmount,
      maximumAmount,
      hideAddToCart,
      hideBuyNow,
...
export default function VisibilityPage() {
  ...
  // State to manage form changes - initialized from loader data with explicit typing
