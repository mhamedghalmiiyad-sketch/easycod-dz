...
// Note: Server-only imports have been moved into the loader function.
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  // ✅ Server-only imports moved inside the loader
  const { authenticate } = await import("../shopify.server");
  const { wilayaInfoMap } = await import("../utils/algeria-locations.server");
  ...
  try {
    ...
    switch (apiProvider) {
      ...
      // New cases for Procolis-based APIs
      case "zrexpress":
      case "procolis": {
        const apiKey = url.searchParams.get("apiKey");
        if (!apiKey) {
          return json(
            { error: `API Key is required for ${apiProvider} provider` },
            { status: 400 }
          );
        }

        console.log(`Fetching rates from ${apiProvider}...`);
        const API_URL = "https://procolis.com/api_v1/tarification";
        ...
        const data = await response.json();
        // Adjust mapping based on the actual API response structure
        const rates = data.rates || data.tarifs || [];

        deliveryRates = rates.map((rate: any) => {
          const wilayaId = rate.wilaya_id || rate.id;
          const wilayaInfo = wilayaInfoMap.get(wilayaId);
          return {
            wilaya_id: wilayaId,
            wilaya_name_fr: wilayaInfo?.name_fr ?? rate.wilaya_name ?? `Wilaya ${wilayaId}`,
            wilaya_name_ar: wilayaInfo?.name_ar ?? `ولاية ${wilayaId}`,
            tarif: parseFloat(rate.home_price || rate.tarif || 0),
            tarif_stopdesk: parseFloat(rate.desk_price || rate.tarif_stopdesk || 0),
          };
        });
        break;
      }
