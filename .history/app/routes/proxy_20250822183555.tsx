...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    const shippingAddress = {
      ...
      lastName: lastName || fullName.split(' ').slice(1).join(' ') || fullName.split(' ')[0], // Fallback for single-word names
      address1: formData.get("address") as string,
      address2: formData.get("address2") as string,
      city: formData.get("commune") as string || formData.get("city") as string, // Prioritize commune
      province: formData.get("wilaya") as string || formData.get("province") as string, // Prioritize wilaya
