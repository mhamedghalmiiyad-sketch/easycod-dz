...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const proxyHtml = `
    ...
    <html>
      ...
      <body>
        <div class="easycod-form-container" data-section-id="codform-proxy"></div>
        <script id="easycod-form-config-codform-proxy" type="application/json">
          ${JSON.stringify({
            formFields: JSON.parse(formFields || '[]'),
            formStyle: JSON.parse(formStyle || '{}'),
            submitUrl: `https://${shop}/apps/codform/proxy`,
        ...
        <script>
          (function() {
            ...
            const { 
              TextField, Button, Box, Alert, Typography, CircularProgress,
              Select, MenuItem, FormControl, InputLabel, Card, CardContent
            } = MaterialUI;

            function FormApp({ config }) {
              ...
              if (!cart || cart.item_count === 0) {
                return React.createElement(Alert, { severity: 'info' }, 'Your cart is empty.');
              }
              ...
              return React.createElement(Box, {
                ...
                // Cart summary
                React.createElement(Typography, { key: 'title', variant: 'h5', mb: 2 }, 'Complete Your Order'),
                ...
                cart.items.map((item, index) => 
                  React.createElement(Box, {
                    ...
                    React.createElement(Typography, { key: 'title' }, item.product_title),
                    React.createElement(Typography, { key: 'qty' }, `Qty: ${item.quantity}`),
                    React.createElement(Typography, { key: 'price' }, 
                ...
                // Form fields based on config
                config.formFields.filter(f => f.enabled && f.type !== 'submit').map(field => {
                  if (field.type === 'email' || field.type === 'text' || field.type === 'tel') {
                    return React.createElement(TextField, {
                      key: field.id,
                      fullWidth: true,
                      margin: 'normal',
                      label: field.settings.label || field.id,
                ...
                // Error message
                submitError && React.createElement(Alert, { key: 'error', severity: 'error', sx: { mt: 2 } }, submitError),
            ...
            document.addEventListener('DOMContentLoaded', () => {
              const container = document.querySelector('.easycod-form-container');
              const configElement = document.getElementById('easycod-form-config-codform-proxy');
              
              if (container && configElement) {
                try {
                  const config = JSON.parse(configElement.textContent);
                  const root = ReactDOM.createRoot(container);
                  root.render(React.createElement(FormApp, { config }));
                } catch (error) {
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    const shippingAddress = {
      ...
      city: formData.get("commune") as string || formData.get("city") as string || 'Unknown',
      province: formData.get("wilaya") as string || formData.get("province") as string,
