// app/utils/riskScoring.server.ts
// This file contains all the server-only business logic for calculating
// the risk score. It is not bundled for the client.

// Mock the database for a self-contained example.
// In a real app, this would be an actual database client import.
const db = {
  orderTracking: {
    findMany: async (options: any) => {
      console.log('Querying mock database with options:', options);
      return []; // Return an empty array for the mock data
    },
  },
};
...
export async function calculateRiskScore(
  ...
  // The database import is now correctly placed within this server-only
  // function, ensuring it's never bundled for the client.
...
async function checkRTOHistory(factors: RiskFactors, shopId: string) {
  // The database import is also correctly moved here.
  // const db = (await import("../db.server")).default;
