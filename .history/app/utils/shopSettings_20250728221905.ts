...
export async function upsertShopSettings(
  ...
  const dataToUpsert = { ...updates };

  // Convert any object/array settings to JSON strings before saving
  if (dataToUpsert.formFields && typeof dataToUpsert.formFields !== 'string') {
    dataToUpsert.formFields = stringifyJsonField(dataToUpsert.formFields);
  }
  if (dataToUpsert.formStyle && typeof dataToUpsert.formStyle !== 'string') {
    dataToUpsert.formStyle = stringifyJsonField(dataToUpsert.formStyle);
  }
  if (dataToUpsert.pixelSettings && typeof dataToUpsert.pixelSettings !== 'string') {
    dataToUpsert.pixelSettings = stringifyJsonField(dataToUpsert.pixelSettings);
  }
  if (dataToUpsert.visibleProducts && typeof dataToUpsert.visibleProducts !== 'string') {
    dataToUpsert.visibleProducts = stringifyJsonField(dataToUpsert.visibleProducts);
  }
  if (dataToUpsert.generalSettings && typeof dataToUpsert.generalSettings !== 'string') {
    dataToUpsert.generalSettings = stringifyJsonField(dataToUpsert.generalSettings);
  }

  // The create payload needs defaults for any fields not in `updates`.
  const createPayload = {
    formFields: "[]",
    formStyle: "{}",
    pixelSettings: "{}",
    ...
    visibleProducts: "[]",
    generalSettings: JSON.stringify(DEFAULT_GENERAL_SETTINGS),
    ...dataToUpsert, // The provided updates will override the defaults.
    shopId: shopId, // This links it to the session via the foreign key
  };
