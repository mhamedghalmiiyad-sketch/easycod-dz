...
async function sendAbandonedCartReminder(cart: any, settings: any) {
  // Handle case where cart data might not be available
  let cartData = null;
  try {
    cartData = JSON.parse(cart.cartData);
  } catch (error) {
    console.warn('Cart data not available for abandoned cart reminder:', error);
    cartData = { total_price: 0, currency: 'DZD', items: [] };
  }
  ...
  const replacements = {
    ...
    '{cart_total}': cartData ? formatCurrency(cartData.total_price / 100, cartData.currency) : '0.00',
    '{cart_items}': cartData ? formatCartItems(cartData.items) : 'No items available',
