...
interface TkoneMessage {
  role: 'system' | 'user' | 'assistant';
  content: string;
}

interface TkoneResponse {
  choices: Array<{
    message: {
      content: string;
      role: string;
...
export class TkoneAgent {
  private apiKey: string;
  private apiUrl: string;
  private messages: TkoneMessage[];
  ...
  async ask(prompt: string, context?: string): Promise<string> {
    try {
      ...
      const response = await axios.post<TkoneResponse>(
        this.apiUrl,
        {
...
Provide the optimized version with explanations of the improvements.`;
  ...
  getHistory(): TkoneMessage[] {
    return [...this.messages];
  }
}

// Export a singleton instance
export const tkoneAgent = new TkoneAgent();
