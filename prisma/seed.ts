...
// Your data converted to a JavaScript array of objects
const citiesData = [
  {
    id: "22",
    commune_name: "تيمقتن",
...
async function main() {
  ...
  const existingCount = await prisma.algeriaCities.count();
  if (existingCount > 0) {
    console.log(`Algeria cities data already exists (${existingCount} records). Skipping seed.`);
    return;
  }

  // Using createMany for efficient bulk insertion
  const formattedData = citiesData
    .filter(city => city.id) // Filter out entries with an empty id
    .map(city => ({
      ...city,
  ...
  // Using createMany for efficient bulk insertion
  const result = await prisma.algeriaCities.createMany({
    data: formattedData, // Use the corrected data
  });
